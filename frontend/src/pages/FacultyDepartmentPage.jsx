...
export default function FacultyDepartmentPage() {
  ...
  useEffect(() =>{
    ...
    async function fetchDepartments() {
      try {
        ...
        const userDeptMatch = list.find(d =>d.code === userDept ||
          d.name === userDept ||
  ...
  return (
    <Layout title="Departments"><div style={{ paddingBottom: '40px' }}>{/* Action Button */}
        <div style={{ marginBottom: '24px', display: 'flex', justifyContent: 'flex-end' }}><button
              ...
              onMouseLeave={(e) =>e.target.style.background = '#06b6d4'}
            >+ Add Department
            </button></div>{/* Stats Grid - Department Statistics */}
        <KpiGrid><KpiCard
            icon="domain"
            label="Departments"
          ...
          /><KpiCard
            ...
            colorScheme="emerald"
          /></KpiGrid>{/* Department List and Details */}
        <div className="grid grid-cols-1 md:grid-cols-[300px_1fr] gap-6">{/* Department Sidebar List */}
          <div style={{
            background: '#ffffff',
            borderRadius: '12px',
            overflow: 'hidden',
              ...
              fontSize: '14px'
            ...
            </div><div style={{ overflow: 'auto', flex: 1 }}>{loading ? (
                <div style={{ padding: '16px', textAlign: 'center', color: '#9ca3af' }}>Loading...
                </div>) : (
                departments.map((dept) =>{
                  ...
                  return (
                    ...
                    ><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><div style={{ fontSize: '13px', fontWeight: '600', color: selectedDept?.id === dept.id ? '#276221' : '#1f2937', marginBottom: '2px' }}>{dept.code}
                        </div>{isUserDept && (
                          <span style={{ 
                            fontSize: '9px', 
                            background: '#dcfce7', 
                      ...
                      </div><div style={{ fontSize: '12px', color: '#6b7280', lineHeight: '1.2' }}>{dept.name.split(' & ')[0]}
                      </div></button>);
                })
              )}
            </div></div>{/* Department Details */}
          {selectedDept && (
            <div style={{
              background: '#ffffff',
              borderRadius: '12px',
              ...
              <div style={{ marginBottom: '24px', borderBottom: '1px solid #e5e7eb', paddingBottom: '16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '12px' }}><div><div style={{ fontSize: '12px', fontWeight: '600', color: '#276221', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>{selectedDept.code}
                    </div><h2 style={{ fontSize: '24px', fontWeight: '700', color: '#1f2937', margin: 0 }}>{selectedDept.name}
                    </h2></div><div style={{ display: 'flex', gap: '8px' }}><button 
                    ...
                    </button></div></div><p style={{ fontSize: '14px', color: '#6b7280', margin: 0 }}>{selectedDept.description}
                </p></div>{/* Stats Grid for Selected Dept */}
              <div style={{
                display: 'grid',
                ...
                marginBottom: '24px'
                  ...
                  borderRadius: '10px',
                  padding: '16px'
                }}><div style={{ fontSize: '11px', fontWeight: '600', color: '#16a34a', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>Faculty Members
                  </div><div style={{ fontSize: '28px', fontWeight: '700', color: '#15803d' }}>{selectedDept.totalFaculty}
                  </div></div><div style={{
                  background: '#fdf2f8',
                  border: '1px solid #fbcfe8',
                  borderRadius: '10px',
                  padding: '16px'
                }}><div style={{ fontSize: '11px', fontWeight: '600', color: '#ec4899', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>Students
                  </div><div style={{ fontSize: '28px', fontWeight: '700', color: '#be185d' }}>{selectedDept.totalStudents}
                  </div></div><div style={{
                  background: '#eff6ff',
                  border: '1px solid #bfdbfe',
                  borderRadius: '10px',
                  padding: '16px'
                }}><div style={{ fontSize: '11px', fontWeight: '600', color: '#276221', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>Courses Offered
                  </div><div style={{ fontSize: '28px', fontWeight: '700', color: '#1e4618' }}>{selectedDept.courses}
                  </div></div><div style={{
                  background: '#ecf5ff',
                  border: '1px solid #a5f3fc',
                  borderRadius: '10px',
                  padding: '16px'
                }}><div style={{ fontSize: '11px', fontWeight: '600', color: '#3d8b30', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>Head of Department
                  </div><div style={{ fontSize: '14px', fontWeight: '600', color: '#0369a1', lineHeight: '1.4' }}>{selectedDept.head}
                  </div></div></div>{/* Contact & Location */}
              <div style={{
                background: '#f9fafb',
                borderRadius: '10px',
                padding: '16px',
                marginBottom: '24px'
              }}><h3 style={{ fontSize: '14px', fontWeight: '600', color: '#374151', marginBottom: '12px', margin: '0 0 12px 0' }}>Contact Information
                </h3><div style={{
                  display: 'grid',
                  gridTemplateColumns: '1fr 1fr',
                  gap: '16px'
                }}><div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}><Mail size={18} style={{ color: '#276221', marginTop: '2px', flexShrink: 0 }} /><div><div style={{ fontSize: '12px', color: '#6b7280', fontWeight: '600', marginBottom: '2px' }}>Email</div><a href={`mailto:${selectedDept.email}`} style={{ fontSize: '14px', color: '#276221', textDecoration: 'none' }}>{selectedDept.email}
                      </a></div></div><div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}><span style={{ color: '#276221', marginTop: '2px', flexShrink: 0, fontWeight: '600' }}></span><div><div style={{ fontSize: '12px', color: '#6b7280', fontWeight: '600', marginBottom: '2px' }}>Phone</div><a href={`tel:${selectedDept.phone}`} style={{ fontSize: '14px', color: '#276221', textDecoration: 'none' }}>{selectedDept.phone}
                      </a></div></div><div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', gridColumn: '1 / -1' }}><MapPin size={18} style={{ color: '#276221', marginTop: '2px', flexShrink: 0 }} /><div><div style={{ fontSize: '12px', color: '#6b7280', fontWeight: '600', marginBottom: '2px' }}>Location</div><div style={{ fontSize: '14px', color: '#374151' }}>{selectedDept.location}</div></div></div></div></div></div>)}
        </div>{/* Modals */}
        <AddDepartmentModal
          isOpen={isAddOpen}
